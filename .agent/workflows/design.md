...
A página completa será criada APENAS quando o usuário usar `/build` após aprovar o `/layout`.
...
3. Como visualizar: http://localhost:8888
...
- O usuário deve aprovar antes de prosseguir para `/layout`
...
3. Forneça o link para visualizar: http://localhost:8888
4. Pergunte se quer ajustar algo
5. Sugira a próxima etapa: "Quando o design estiver aprovado, use `/layout` para criar a especificação detalhada de todas as seções."
...
- NUNCA crie o layout completo
- NUNCA execute ações sem o usuário solicitar explicitamente
- Após entregar a demonstração e sugerir o próximo passo, AGUARDE instrução do usuário
- Se o usuário não der instrução, NÃO faça nada
- Se o usuário pedir ajustes no design, faça os ajustes e PARE novamente
